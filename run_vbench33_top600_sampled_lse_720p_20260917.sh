#!/usr/bin/env bash
# Full VBench33 main experiment: Top600 + sampled-LSE at 1280x720.
...
PROMPT_FILE="/cnic/work/liutt/mywork/attention/Sparse-VideoGen/data/vbench_data/vbench_33_prompts.txt"
RISK_FILE="${REPO_ROOT}/importanthead/risk_sets/risk_top600.txt"
OUTPUT_DIR="${OUTPUT_DIR:-/cnic/work/liutt/mywork/attention_time/res/hymor_vbench33_top600_sampled_lse_720p_20260917/vbench_33/HunyuanVideo/seed0_tilekratio0.30_dynamicTrue_dcrt0.10_totaltopp0.90_scoresampled_lse_temp1.0_mink20_maxk32_promote24_resbackendmicro_rodecacheFalse_parallelFalse_hilbert3d_720_routecacheTrue_coreonlyFalse_directcsrTrue_ctamul0_cache12}"

if [ ! -f "$PROMPT_FILE" ] || [ "$(wc -l < "$PROMPT_FILE")" -ne 33 ]; then
    echo "ERROR: VBench33 prompt file is missing or does not contain 33 lines: $PROMPT_FILE" >&2
    exit 1
fi
if [ ! -f "$RISK_FILE" ]; then
...
mkdir -p "$OUTPUT_DIR"
cd "$REPO_ROOT"
echo "[$(date '+%F %T')] START Top600 + sampled-LSE, VBench33 0--32, 1280x720"
...
echo "[$(date '+%F %T')] DONE Top600 + sampled-LSE, VBench33 1280x720"
